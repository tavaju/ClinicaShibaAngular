...
export class AuthService {
      ...
      if (dashboardUrl) {
        ...
        if (normalizedRole === 'CLIENT') {
          ...
          if (username && dashboardUrl.includes('cliente/home')) {
            console.log(
              `Redirecting CLIENT to: ${dashboardUrl} with email: ${username}`
            );
  ...
  /**
   * Logs in a veterinarian user
   * @param credentials The login credentials for the veterinarian
   * @returns Observable with the login response
   */
  loginVet(credentials: any): Observable<any> {
    return this.http.post('/api/vet/login', credentials).pipe(
      tap((response: any) => {
        // ...guardar token, etc...
        if (response && response.veterinario && response.veterinario.id) {
          localStorage.setItem(
            'currentVetId',
            response.veterinario.id.toString()
          );
        }
      })
    );
